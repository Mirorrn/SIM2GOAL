...
class Config:
    def __init__(self):
        ...
        self.experiment_name = 'GFLOW-TrajNet_Clip_Clamp'
        path= self.DIR + self.experiment_name
        if not os.path.exists(path):
            os.makedirs(path)
        self.model_path = path
        # Dataset options
        self.dataset_name = 'wildtrack'   # Choose dataset split only for single split training
        ...
        if not self.trajnet:
           ...
           self.sampler_checkpoint_start_from = self.DIR + 'GFLOW-ETHandUCY_sampler-'
            ...
            self.sampler_checkpoint_start_from = self.DIR + 'GFLOW-TrajNet_Clip_Clamp-'
            self.GFlow_checkpoint_start_from   = self.DIR + 'GFLOW-TrajNet_Clip_clamp-'
