...
def seq_collate(data):
    ...
    nei_num_index = torch.zeros([obs_traj.shape[1],obs_traj.shape[1]])
    nei_num = torch.zeros([obs_traj.shape[1]])
    for i, (start, end) in enumerate(seq_start_end):
        start = start.item()
        end = end.item()
        nei_num_index[start:end, start:end] = 1.
        nei_num[i] = end - start
    nei_num_index = nei_num_index.fill_diagonal_(0).float()
        ...
        for path in all_files:
            ...
            for idx in range(0, num_sequences * self.skip + 1, skip):
                ...
                for _, ped_id in enumerate(peds_in_curr_seq):
                    ...
                    if config.nabs:
                        rel_curr_ped_seq = curr_ped_seq / 28.# - np.expand_dims(curr_ped_seq[:, self.obs_len-1], -1)
                    else:
